...
exports.getUnavailableCapabilities = exports.parseCapabilities = void 0;
...
var getUnavailableCapabilities = function getUnavailableCapabilities(features, coins, support) {
  ...
  support.forEach(function (s) {
    if (s.min && (0, _versionUtils.versionCompare)(s.min[fw[0] - 1], fw) > 0) {
      s.excludedMethods.forEach(function (m) {
        list[m] = s.coin || 'update-required';
      });
    }

    if (s.max && (0, _versionUtils.versionCompare)(s.max[fw[0] - 1], fw) < 0) {
      s.excludedMethods.forEach(function (m) {
