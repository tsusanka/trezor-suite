...
const calculateXPosition = (x: number, offset: number = 0) => `calc(${x}px - ${x / 2}px + ${offset}px)`;
const calculateXPositionRight = (x: number, offset: number = 0) => `calc(${x}px + 25% + ${offset}px)`;
...
const getTooltipXPosition = (x: number, width: number): string => {
    ...
    } else if (x >= width - OFFSET_LIMIT_HORIZONTAL) {
...
const getTooltipArrowXPosition = (x: number, width: number): string => {
    return x <= OFFSET_LIMIT_HORIZONTAL
        ? `left: ${calculateXPosition(x, -30)};`
        : x >= width - OFFSET_LIMIT_HORIZONTAL
            ? `left: calc(75% + 1px);`
            : `left: 50%;`;
};
...
const CustomTooltipWrapper = styled.div<{
    ...
    ${props => props.positionX >= props.boxWidth - OFFSET_LIMIT_HORIZONTAL && `position: absolute; right: 0;`}
